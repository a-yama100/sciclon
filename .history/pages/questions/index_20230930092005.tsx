...
const QuestionsPage: React.FC = () => {
    ...
    const handleUpdate = async () => {
        ...
        // 科目に関連する用語と解説をランダムに取得
        const response = await fetch(`/api/questions/randomTermFromGlossary?glossary=${selectedSubject}`);
        const data = await response.json();
        setQuestions([data]);
    };
    ...
    return (
        <div>
            ...
            <div>
                {questions.map((term) => (
                    <div align="center"　key={term.id}>
                        <p ><button onClick={handleUpdate}>更新</button></p>
