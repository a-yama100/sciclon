...
const UserDashboard: React.FC = () => {
    ...
    const handleCertificationChange = async (cert: Certification) => {
        ...
        try {
            // 選択された認定が「aws_associate_solutions_architect_glossary」の場合、新しいエンドポイントを呼び出す
            if (cert.aws_certifications_name === 'aws_associate_solutions_architect_glossary') {
                const response = await fetch(`/api/questions/randomTermFromCertification`);
                if (!response.ok) {
                    throw new Error("Failed to fetch random term.");
                }
                const data = await response.json();
                setQuestions([data]);  // 1つのランダムな用語をquestionsとしてセット
            } else {
                // その他の認定の場合、以前のエンドポイントを呼び出す
